...
$(function(){
 ...
 //jQuery 안에는 el 태그를 같이 못씀 , 따로 변수를 선언해주고 나서 사용해야 함
   //jquery ajax 해당 게시글에 대한 댓글 조회 요청
   //해당 게시글의 번호를 전송함
   var bid = ${ board.bid };	// el 의 값을 변수에 대입
   var loginUser = "${ sessionScope.loginUser.id }";   //로그인한 회원 아이디 변수에 대입	 --> el임 
	$.ajax({
		url : "${ pageContext.request.contextPath }/rlist.do", /* 절대경로 */
		type : "post",
		data : { ref_bid: bid }, //보내는 방식	전송값에 변수 사용
		dataType : "json",
		success : function(data) {
			...
			for ( var i in json.list) {
				...
				if(loginUser === json.list[i].rwriter){
					...
					+ "<input type='hidden' name='rid' value='" + json.list[i].rid + "'>"	//따옴표 안에는 el태그 사용가능
...
function replyDelete(rid){
	location.href = "${ pageContext.request.contextPath }/rdel.do?rid=" + rid + "&bid=${ board.bid }";	// 앞에 꼭 절대경로여야함...
}
...
<tr><th>제 목</th><td>${ board.btitle }</td></tr>
<tr><th>작성자</th><td>${ board.bwriter }</td></tr>
<tr><th>내 용</th><td>${ board.bcontent }</td></tr>
<tr><th>첨부파일</th>
<td>
...
<table id="rlistTbl" cellspacing="0" cellpadding="5" border="1" align="center"></table>
</div>
<hr>

<%-- 댓글달기 폼 영역  --%>
<div id="replyDiv">
<form action="rinsert.do" method="post" > <!-- 파일 첨부 할게 없으니까 enctype="multipart/form-data" 지우기  -->
